...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        # Probability of class 0 is complement
        prob_class_0 = 1.0 - prob_class_1
        # Stack probabilities into a matrix (N, 2)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        # Compute scores (logits): z = X * w
        scores = X_batch.dot(self.w)

        # Compute sigmoid: σ(z) = 1 / (1 + exp(-z))
        sigmoid = 1.0 / (1.0 + np.exp(-scores))

        # Compute logistic loss: L = -1/N * Σ[y*log(σ) + (1-y)*log(1-σ)]
        # For numerical stability, use the formula:
        # L = -1/N * Σ[y*scores - log(1 + exp(scores))]
        loss = -np.sum(y_batch * np.log(sigmoid + 1e-8) + (1 - y_batch) * np.log(1 - sigmoid + 1e-8))

        # Compute gradient: dL/dw = X^T * (σ - y)
        dw = X_batch.T.dot(sigmoid - y_batch)
